# 5 Tjänstedomänens meddelandemodeller - itintegration: engagementindex v1.0.9
...
Följande tabell beskriver innehållet i en engagemangspost: Varje tillämpande tjänstedomän preciserar betydelsen för de fält som är markerade med asterisk i informationsmodellen.
...
| Service domain | Den tjänstedomän som förekomsten avser. | URN på formen:::. Ex: ”riv:crm:scheduling” | 1..1 | Nationellt index kan innehålla tjänstedomäner som är nationellt fastställda. Huvudmanna-index kan även tillåta icke-nationella domäner. | Del av instansens unikhet |
...
Person- eller samordningsnummer enligt skatteverkets definition (12 tecken). Nationellt reservnummer enligt Ineras definition (12 tecken), se referens Ref3 Validering enligt format(xml-regexp): [0-9]{8}[0-9A-Zptf]{4}
