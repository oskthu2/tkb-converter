					...
					<xsl:for-each select="testcase|section">
						...
						<p>
							<xsl:choose>
								...
								<xsl:when test="starts-with(@id, '1.7 ')">Verifierar att responsen är ett <b>Soapfault.</b> Detta testfall kräver att tjänsteproducenten skapar förutsättningar för ett internt fel att uppstå.<br/>
								<b>Manuell kontroll av svarsmeddelandet krävs för att säkerställa att meddelandet inte innehåller personuppgifter eller att medföljande log-id är spårbart till patienten.</b><br/>
								...
								<xsl:when test="starts-with(@id, '7.3 ')">Verifierar att tjänsteproducenten returnerar samma svar oavsett vilket tjänstekonsumentHSAId (HTTP-headern "x-rivta-original-serviceconsumer-hsaid") som anges.<br/></xsl:when>
								<xsl:otherwise>
