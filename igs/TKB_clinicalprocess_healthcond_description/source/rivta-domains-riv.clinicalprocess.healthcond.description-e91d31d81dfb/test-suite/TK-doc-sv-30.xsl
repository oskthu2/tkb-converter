	...
	<xsl:template match="/testsuite">
		<html>
			...
			<body>
				...
				Detta underlag för godkännande kommer att granskas och eventuella avvikelser rapporteras till Ineras avvikelsegrupp. Avvikelserna granskas av avvikelsegruppen och kan leda till tre utfall:<br/>
				<ul>
					...
					<xsl:for-each select="testcase|section">
						...
						<p>
							<xsl:choose>
								...
								<xsl:when test="starts-with(@id, '1.1.3 ')">Filtrering. Testfall för filtrering på Nationellt reservID. Ersätt patientId med det NRID som du vill filtrera på.</xsl:when>
								<xsl:when test="starts-with(@id, '1.1.4 ')">Filtrering. Testfall för filtrering på reservnummer. Ersätt patientId med det reservnummer som du vill filtrera på och ersätt patientIdType med lokalt definierat OID för reservnummer.</xsl:when>
								...
								<xsl:when test="starts-with(@id, '3.1 ')">Verifierar att alla returnerade poster innehåller elementen <b>accountableCareUnit</b> och <b>accountableHealthcareProvider</b>, som krävs för PDL-loggning.</xsl:when>
								<xsl:when test="starts-with(@id, '3.2')">Verifierar att alla returnerade poster innehåller tidpunkt då informationen registrerades. Element <b>blockComparisonTime</b>.</xsl:when>
								...
								<xsl:when test="starts-with(@id, '5.1 signed')">Verifierar att tjänsteproducenten kan returnera en signerad post. Element <b>signature</b>.</xsl:when>
								<xsl:when test="starts-with(@id, '5.2 unsigned')">Verifierar att tjänsteproducenten kan returnera en osignerad post. Element <b>signature</b>.</xsl:when>
