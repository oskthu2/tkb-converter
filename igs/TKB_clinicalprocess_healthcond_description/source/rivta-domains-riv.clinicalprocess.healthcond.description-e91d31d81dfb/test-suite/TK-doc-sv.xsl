					...
					<xsl:for-each select="testcase|section">
						...
						<p>
							<xsl:choose>
								...
								<xsl:when test="starts-with(@id, '3.2 AuthorTime')">Verifierar att en av de returnerade posterna innehåller tidpunkt då informationen registrerades. Element <b>authorTime</b>.</xsl:when>
								<xsl:when test="starts-with(@id, '3.2 BlockComparisonTime')">Verifierar att alla returnerade poster innehåller tidpunkt då informationen registrerades. Element <b>blockComparisonTime</b>.</xsl:when>
								...
								Testfallet är ej applicerbart för system som inte implementerat denna typ av filtreringar.</xsl:when>
